...
my_table4_filtered = my_table3.filter(lambda x: x['EU'] == 'yes').filter(lambda x: (x['coastline']) == 'no')
...
country_list = [x['country'] for x in countries]
city_list = [x['country'] for x in cities]
country_list = [x for x in country_list if x in city_list]
print("Max latitude")
for i in country_list:
    country = my_table3.filter(lambda x: x['country'] == i)
    print(i, country.aggregate(lambda x: max(x), 'latitude'))
print()
print("Min latitude")
for i in country_list:
    country = my_table3.filter(lambda x: x['country'] == i)
    print(i, country.aggregate(lambda x: min(x), 'latitude'))
